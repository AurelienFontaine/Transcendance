		...
		try {
			...
			if (firstTime)
			{
				...
				try {
					...
					console.log("Print verif err on insert");
					console.error(err);
					throw (err);
				}
			}
			// const token = fastify.jwt.sign({ id: user.id, username: user.username, mustChangePassword: true}, process.env.JWT_KEY!, { expiresIn: "1h" });
			return {
				token,
				name: user.name,
