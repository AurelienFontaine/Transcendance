...
export class DashboardManager {
  ...
  async loadDashboardData(userName: string): Promise<DashboardData> {
    ...
    try {
      ...
      // Use the actual user name from the /me endpoint instead of localStorage
      const actualUserName = user.name || userName;

      // Fetch enhanced statistics
      const statsUrl = `${apiBase()}/users/${encodeURIComponent(actualUserName)}/enhanced-stats`;
      ...
      let statsData;
      if (!statsRes.ok) {
        console.warn(`Enhanced stats failed (${statsRes.status}), trying basic stats...`);
        
        // Fallback to basic stats if enhanced stats fails
        const basicStatsUrl = `${apiBase()}/users/${encodeURIComponent(actualUserName)}/stats`;
        
        const basicStatsRes = await fetch(basicStatsUrl, {
          headers: { Authorization: `Bearer ${token}` }
        });
        
        if (!basicStatsRes.ok) {
          throw new Error(`Failed to fetch both enhanced and basic stats: ${basicStatsRes.status} ${basicStatsRes.statusText}`);
        }
        
        const basicStats = await basicStatsRes.json();
        
        // Create a compatible statsData structure
        statsData = {
          basic: basicStats,
          enhanced: {
            totalPointsScored: 0,
      ...
      // Fetch match history
      const historyRes = await fetch(`${apiBase()}/users/${encodeURIComponent(actualUserName)}/history`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      
      if (!historyRes.ok) {
        throw new Error(`Failed to fetch match history: ${historyRes.status} ${historyRes.statusText}`);
      }
      
      const historyData = await historyRes.json();
      const matches = historyData.matches || [];

      // Validate statsData structure
      if (!statsData || !statsData.basic || !statsData.enhanced) {
        throw new Error('Invalid stats data structure received from server');
      }

      // Use enhanced stats from backend
      const stats: UserStats = {
        wins: statsData.basic.wins || 0,
        losses: statsData.basic.losses || 0,
        total: statsData.basic.total || 0,
        winrate: statsData.basic.winrate || 0,
        pointsGagnes: statsData.enhanced.totalPointsScored || 0,
        pointsPris: statsData.enhanced.totalPointsConceded || 0,
        ratioMarque: statsData.enhanced.pointsRatio || 0,
        ratioPris: 100 - (statsData.enhanced.pointsRatio || 0),
        ppmMarque: statsData.basic.total > 0 ? (statsData.enhanced.totalPointsScored || 0) / statsData.basic.total : 0,
        ppmPris: statsData.basic.total > 0 ? (statsData.enhanced.totalPointsConceded || 0) / statsData.basic.total : 0,
        currentStreak: statsData.enhanced.currentStreak || 0,
        bestStreak: statsData.enhanced.bestStreak || 0,
        worstStreak: statsData.enhanced.worstStreak || 0,
        averageScore: statsData.enhanced.averageScore || 0,
        bestScore: statsData.enhanced.bestScore || 0,
        worstScore: statsData.enhanced.worstScore || 0
      };
      ...
      <div class="grid grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-gradient-to-br from-green-600 to-green-700 p-4 rounded-lg shadow-lg">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-green-100 text-sm font-medium">${t('enhancedProfile.cards.winRate')}</p>
              <p class="text-2xl font-bold text-base-content">${stats.winrate.toFixed(1)}%</p>
            </div>
        ...
        <div class="bg-gradient-to-br from-blue-600 to-blue-700 p-4 rounded-lg shadow-lg">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-blue-100 text-sm font-medium">${t('enhancedProfile.cards.totalGames')}</p>
        ...
        <div class="bg-gradient-to-br from-purple-600 to-purple-700 p-4 rounded-lg shadow-lg">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-purple-100 text-sm font-medium">${t('enhancedProfile.cards.currentStreak')}</p>
              <p class="text-2xl font-bold text-base-content">${stats.currentStreak > 0 ? '+' : ''}${stats.currentStreak}</p>
            </div>
        ...
        <div class="bg-gradient-to-br from-orange-600 to-orange-700 p-4 rounded-lg shadow-lg">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-orange-100 text-sm font-medium">${t('enhancedProfile.cards.avgScore')}</p>
              <p class="text-2xl font-bold text-base-content">${stats.averageScore.toFixed(1)}</p>
            </div>
  ...
  renderWinLossChart(containerId: string, stats: UserStats): void {
    if (typeof Chart === 'undefined') {
      console.error('Chart.js is not loaded, cannot render win/loss chart');
      return;
    }

    const container = document.getElementById(containerId);
    if (!container) {
      console.error(`Win/Loss chart container '${containerId}' not found`);
      return;
    }
    ...
    const chart = new Chart(ctx, {
      type: 'doughnut',
      data: {
        labels: [t('enhancedProfile.matchHistory.wins'), t('enhancedProfile.matchHistory.losses')],
        datasets: [{
  ...
  renderPerformanceTrend(containerId: string, performanceData: { dates: string[], winRates: number[], scores: number[] }): void {
    if (typeof Chart === 'undefined') {
      console.error('Chart.js is not loaded, cannot render performance trend chart');
      return;
    }

    const container = document.getElementById(containerId);
    if (!container) {
      console.error(`Performance chart container '${containerId}' not found`);
      return;
    }
    ...
    const chart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [
          {
            label: t('enhancedProfile.charts.winRatePercent'),
          ...
          {
            label: t('enhancedProfile.charts.averageScore'),
      ...
      options: {
        ...
        scales: {
          ...
          y: {
            ...
            title: {
              display: true,
              text: t('enhancedProfile.charts.winRatePercent'),
              color: '#e5e7eb'
            }
          },
          y1: {
            ...
            title: {
              display: true,
              text: t('enhancedProfile.charts.averageScore'),
              color: '#e5e7eb'
            }
          }
        },
  ...
  renderMatchHistory(containerId: string, matches: MatchData[]): void {
    ...
    if (matches.length === 0) {
      container.innerHTML = `<p class="text-base-content text-center py-8">${t('enhancedProfile.matchHistory.noMatches')}</p>`;
      return;
    }

    const html = matches.map(match => {
      const isVictory = match.result === 'W';
      const resultClass = isVictory ? 'bg-success border-success text-success' : 'bg-error border-error text-error';
      ...
      return `
        <div class="bg-base-100 rounded-lg p-4 border-l-4 ${isVictory ? 'border-success' : 'border-error'}">
          <div class="flex items-center justify-between">
            ...
            <div class="text-right">
              ...
              <div class="text-sm ${isVictory ? 'text-success' : 'text-error'}">
                ${isVictory ? t('enhancedProfile.matchHistory.victory') : t('enhancedProfile.matchHistory.defeat')}
              </div>
            </div>
          </div>
        </div>
